...
- `pages/7_Signal_Map.py` — filter trips by date range, avg RSRP range (dual-handle slider), duration range, and US state (multiselect, filters on first_cell or last_cell within state bounding box). Routes rendered as folium PolyLines coloured red→green using the data's actual p10/p90 RSRP percentiles as scale bounds (loaded at startup). Cell tower dots hidden below zoom 10 via a JS `_ZoomLayer` MacroElement. All matched-trip tables across pages 1, 3, 4, 5, and 7 include a JSON download button.

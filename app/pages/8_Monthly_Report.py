...
ROUTE_SAMPLE = 3000   # max PolyLines on map
...
table = pd.DataFrame([{
    ...
    "start_lat":         round(float(r["start_lat"]), 6) if pd.notna(r["start_lat"]) else None,
    "start_lon":         round(float(r["start_lon"]), 6) if pd.notna(r["start_lon"]) else None,
    "end_lat":           round(float(r["end_lat"]),   6) if pd.notna(r["end_lat"])   else None,
    "end_lon":           round(float(r["end_lon"]),   6) if pd.notna(r["end_lon"])   else None,
